 ...
 */
package com.jovana.videoklubzajednicko.domen;
...
@NamedQueries({
    ...
    @NamedQuery(name = "Kopija.findByKopijaid", query = "SELECT k FROM Kopija k WHERE k.kopijaPK.kopijaid = ?1"),
    @NamedQuery(name = "Kopija.findByFilmid", query = "SELECT k FROM Kopija k WHERE k.kopijaPK.filmid = ?1"),
...
public class Kopija implements Serializable {
    ...
    @JsonView(View.FullKopija.class)
